#!/bin/bash
# ERNIE-Image-Turbo Feldlauf, losgelöst von worker2 gegen den laufenden Container
# auf worker1 (rack:8011). Danach Vergleichsseite + Hub. Container am Ende abräumen.
set -u
...
cd "$IMG"
log "=== ERNIE-FIELD START (Eval gegen rack:8011, 8 Steps) ==="
python3 "$IMG/eval/image_eval.py" \
  --endpoint http://10.0.0.6:8011 \
...
log "ERNIE-Container auf rack abräumen (GPU freigeben)"
ssh -o BatchMode=yes gb10-rack 'docker rm -f southbyte-image-ernie' >/dev/null 2>&1 && log "Container entfernt" || log "Container-Entfernen fehlgeschlagen"
