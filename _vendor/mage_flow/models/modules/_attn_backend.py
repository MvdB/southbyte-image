...
def _resolve_fa2() -> Callable[..., Any]:
    # 2026-08-11 southbyte: flash-attn baut auf sm_120/CUDA13 nur via >100min-Multi-Arch-
    # Compile (kein Wheel) → wir bauen es NICHT ins Image. Fehlt es, transparent auf den
    # (bereits vorhandenen) SDPA-Fallback ausweichen, statt beim Laden zu crashen.
    try:
        from flash_attn import flash_attn_varlen_func as _fn
        return _fn
    except ImportError:
        return _resolve_sdpa()
